...
## SENSE Integration (experimental) 🧪

VQASynth includes scaffolding for integrating the [SENSE framework](https://arxiv.org/abs/2605.19289) (*What Makes Synthetic Data Effective in Image Segmentation*, Zhang et al. 2026) — a model-agnostic synthetic data generator that produces images with dense scene composition and fine instance fidelity for downstream segmentation tasks.

The integration point lives at [`vqasynth/sense_integration.py`](vqasynth/sense_integration.py) and exposes:

- `SENSEConfig` — dataclass with the paper's reported hyperparameters (`guidance_scale=7.5`, `num_inference_steps=50`, `min_instances_per_image=8`).
- `SENSEGenerator` — class interface compatible with the existing scene-generation stage. Drop-in compatible with downstream `localize.py` (Molmo + SAM2) and `scene_fusion.py`.
- `standardize_pixel_range` — utility for converting between SENSE's `[0, 1]` diffusion output and Molmo's expected `[0, 255]` uint8 input.

**Current status**: integration interface + pixel-range plumbing + tests are landed. Actual SENSE checkpoint loading is a TODO pending the [SENSE repository's](https://github.com/zhang0jhon/SENSE) API stabilization and license review. Once a checkpoint is wired in, no changes to downstream stages should be required.

Run the integration tests:

```bash
pytest tests/test_sense_integration.py -v
```

> _This integration was drafted by [🔍 Feature Finder](https://huggingface.co/spaces/remyxai/Feature-Finder) — an open-source paper-recommendation pipeline that maps GitHub merge histories to arXiv reading suggestions with PR-ready specs. The recommended paper, implementation plan, and acceptance criteria were generated by Feature Finder; the code was then implemented by a coding agent using that spec as the brief._
